...
function updateFood(foodId, updates) {
  var foods = getAllFoods();
  var changed = false;
  var newFoods = foods.map(function(f) {
    ...
    for (var k2 in updates) {
      if (Object.prototype.hasOwnProperty.call(updates, k2) && k2 !== 'id') {
        copy[k2] = updates[k2];
      }
    }
    return copy;
...
/**
 * Delete a food listing and clean up related data
 */
function deleteFood(foodId) {
  // Remove from foods
  var foods = getAllFoods().filter(function(f) { return f.id !== foodId; });
  saveFoods(foods);

  // Remove interests for this food
  var interests = getAllInterests().filter(function(i) { return i.foodId !== foodId; });
  saveInterests(interests);

  // Remove notifications for this food
  var notifications = getNotifications().filter(function(n) { return n.foodId !== foodId; });
  saveNotifications(notifications);

  // Remove messages for conversations related to this food
  var prefix = foodId + '_';
  var messages = getAllMessages().filter(function(m) {
    return typeof m.conversationId !== 'string' || m.conversationId.indexOf(prefix) !== 0;
  });
  saveMessages(messages);

  // Remove reviews for this food
  var reviews = getAllReviews().filter(function(r) { return r.foodId !== foodId; });
  saveReviews(reviews);
}
