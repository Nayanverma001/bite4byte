...
(function() {
  ...
  function renderDonor() {
    ...
    if (activeListings.length > 0) {
      ...
      myGrid.innerHTML = activeListings.slice(0, 8).map(function(f) {
        ...
        return '<div class="listing-mini-card" data-food-id="' + f.id + '">' +
          '<img src="' + img + '" alt="">' +
          ...
          '<div style="margin-left:auto; display:flex; gap:0.25rem; flex-wrap:wrap;">' +
            '<button type="button" class="btn btn-secondary btn-sm listing-edit" data-edit-food="' + f.id + '">Edit</button>' +
            '<button type="button" class="btn btn-secondary btn-sm listing-delete" data-delete-food="' + f.id + '">Delete</button>' +
          '</div>' +
          '</div>';
      }).join('');
    } else {
    ...
    // Bind delete buttons (once) using event delegation
    function bindListingActions(gridId) {
      var grid = document.getElementById(gridId);
      if (!grid || grid._actionsBound) return;
      grid.addEventListener('click', function(e) {
        var deleteBtn = e.target.closest('[data-delete-food]');
        if (deleteBtn) {
          var foodIdDel = deleteBtn.getAttribute('data-delete-food');
          if (!foodIdDel) return;
          if (!window.confirm('Delete this listing? This will also remove its chats, interests, and reviews.')) return;
          deleteFood(foodIdDel);
          renderDonor();
          return;
        }
        var editBtn = e.target.closest('[data-edit-food]');
        if (editBtn) {
          var foodIdEdit = editBtn.getAttribute('data-edit-food');
          if (!foodIdEdit) return;
          var foodToEdit = getFoodById(foodIdEdit);
          if (!foodToEdit) return;
          window._editingFoodId = foodIdEdit;
          var form = document.getElementById('donorForm');
          document.getElementById('formPanel').classList.remove('hidden');
          document.getElementById('thankYouSection').classList.add('hidden');
          form.foodName.value = foodToEdit.foodName || '';
          form.quantity.value = foodToEdit.quantity || '';
          form.location.value = foodToEdit.location || '';
          form.expiryDate.value = foodToEdit.expiryDate || '';
          form.expiryTime.value = foodToEdit.expiryTime || '';
          form.description.value = foodToEdit.description || '';
          var typeRadio = form.querySelector('input[name="foodType"][value="' + (foodToEdit.foodType || 'vegetarian') + '"]');
          if (typeRadio) typeRadio.checked = true;
          if (foodToEdit.imageUrl && document.querySelectorAll('.preset-img').length) {
            document.getElementById('selectedImageUrl').value = foodToEdit.imageUrl;
            document.querySelectorAll('.preset-img').forEach(function(el) {
              el.classList.toggle('selected', el.dataset.url === foodToEdit.imageUrl);
            });
          }
    ...
    bindListingActions('myListingsGrid');
    bindListingActions('outOfStockGrid');
  ...
  document.getElementById('donorForm').addEventListener('submit', function(e) {
    ...
    if (window._editingFoodId) {
      // Update existing listing
      updateFood(window._editingFoodId, data);
      window._editingFoodId = null;
    } else {
      // Create new listing
      addFood(data);
    }
    ...
    // Re-render listings and out-of-stock sections after add / edit
    renderDonor();
  });
